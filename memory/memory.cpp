...
void memory::uint16_to_bytes(byte* dest, uint16_t value) {
    if constexpr (std::endian::native == std::endian::little) {
        std::memcpy(dest, reinterpret_cast<char*>(&value), 2);
    } else {
...
uint16_t memory::bytes_to_uint16(const byte* src) { 
    uint64_t value = 0;
    if constexpr (std::endian::native == std::endian::little) {
        std::memcpy(reinterpret_cast<char*>(&value), src, 2);
    } else {
...
void memory::uint64_to_bytes(byte* dest, uint64_t value) {
    if constexpr (std::endian::native == std::endian::little) {
...
uint64_t memory::bytes_to_uint64(const byte* src) {
    uint64_t value = 0;
    if constexpr (std::endian::native == std::endian::little) {
